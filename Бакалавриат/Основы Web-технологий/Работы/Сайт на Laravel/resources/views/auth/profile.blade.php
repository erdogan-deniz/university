...
@section('content')
<h1 id="pages_text">Профиль пользователя</h1>
...
<div>
	<form action="{{route ('auth.profile.update') }}" method="POST" enctype="multipart/form-data">
		...
		<input id="Surname" placeholder="Иванов" type="text" id="Send" maxlength="50" name='sername'
			value="{{ auth()->guard()->user()->sername }}">
		<br>
		<br>
		<label style="margin-top: 5px" for="Data">Выберите дату рождения:</label>
		<br>
		<input id="Data" type="date" value="{{ auth()->guard()->user()->date }}" min="1960-01-01" max="2016-01-01"
			id="Send" name='date'>
		<br>
		<br>
		...
		<input id="Login" class="Login" placeholder="examplelogin" type="text" id="Send" maxlength="50" name='login'
			value="{{ auth()->guard()->user()->login }}" readonly>
		...
		<input id="Email" class="Email" placeholder="befree@world.com" type="email" id="Send" maxlength="50"
			name='email' value="{{ auth()->guard()->user()->email }}">
		<br>
		<br>

		<label for="pass3">Новый пароль:<br></label>
		</label>
		<input id="pass2" class="pass2" placeholder="..." type="password" minlength="8" maxlength="50" name='password3'>
		<br>
		<br>

		<label for="pass4">Введите повторно новый пароль:<br></label>
		</label>
		<input id="pass4" class="pass2" placeholder="..." type="password" minlength="8" maxlength="50" name='password4'>
		...
		<input id="phone-mask" placeholder="+7-926-666-66-66" type="text" name='telephone'
			value="{{ auth()->guard()->user()->telephone }}">
		<br>
		<br>
		<label for="credit-mask">Номер карты лояльности</label>
		<input id="credit-mask" placeholder="0000 0000 0000 0000" type="text" name='number'
			value="{{ auth()->guard()->user()->number }}" readonly>
		<br>
		<br>
		...
		<input id="Gender" type="radio" name="gender" id="esc" value="{{ auth()->guard()->user()->gender }}" checked>
		...
		<div class="autocomplete">
			...
			<input class="inpauto" id="countryInput" name="country" value="{{ auth()->guard()->user()->country }}">
		</div>
		<br>
		<br>
		<input id="button" type="submit" value="Обновить данные" name="final"></input>
	</form>
</div>
